...
export default function UserProfile() {
  ...
  if (!user) {
    return (
      <div style={{ textAlign: 'center', padding: '100px 20px', background: '#f8fafc', borderRadius: '16px' }}>
        <h2 style={{ color: 'var(--text-main)' }}>{t('auth_required')}</h2>
        <p style={{ color: 'var(--text-muted)' }}>{t('login_profile_desc')}</p>
      </div>
    );
  }
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSettingsMsg(t('profile_updated_success'));
      } else {
        setSettingsMsg(data.error || t('failed_update_profile'));
      }
    } catch (e) {
      setSettingsMsg(t('network_error') + ': ' + e.message);
    }
  };
  ...
  const handleChangePassword = async () => {
    ...
    if (!currentPassword || !newPassword) {
      setPasswordMsg(t('please_fill_both_fields'));
      return;
    }
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setPasswordMsg(t('password_changed_success'));
        setCurrentPassword('');
        setNewPassword('');
      } else {
        setPasswordMsg(data.error || t('failed_change_password'));
      }
    } catch (e) {
      setPasswordMsg(t('network_error') + ': ' + e.message);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', gap: '40px', minHeight: '600px' }}>
      ...
      <div style={{ flex: 1 }}>
        {activeTab === 'orders' && (
          <div>
            <h2 style={{ fontSize: '2rem', margin: '0 0 30px 0', fontWeight: '800', color: 'var(--text-main)' }}>{t('my_orders')}</h2>
                ...
                {orders.map(order => (
                  <div key={order._id} style={{ background: 'white', borderRadius: '24px', padding: '30px', border: '1px solid var(--border-light)', boxShadow: '0 10px 30px rgba(0,0,0,0.02)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '1px solid var(--border-light)', paddingBottom: '20px', marginBottom: '20px' }}>
                      <div>
                        <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '5px' }}>{t('order_id')}</div>
                        <div style={{ fontWeight: 'bold', color: 'var(--text-main)' }}>#{order._id?.substring(0, 8) || 'N/A'}</div>
                        <div style={{ marginTop: '8px', fontSize: '0.85rem', color: '#64748b' }}>{t('placed_on')} {new Date(order.createdAt).toLocaleDateString()}</div>
                      </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                      {(expandedOrder === order._id ? order.orderItems : order.orderItems.slice(0, 2)).map((item, idx) => (
                        <div key={idx} style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{t('price')}: ${item.price} | Qty: {item.qty}</div>
                          </div>
                      ...
                      {order.orderItems.length > 2 && (
                        ...
                        >
                          {expandedOrder === order._id ? t('show_less') : `+${order.orderItems.length - 2} ${t('more_items')}`}
                        </button>
                      )}
                    </div>
                    ...
                    {order.shippingAddress && (
                      <div style={{ marginTop: '15px', padding: '12px 16px', background: '#f8fafc', borderRadius: '10px', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                        <strong>{t('ship_to')}</strong> {order.shippingAddress.address}, {order.shippingAddress.city}, {order.shippingAddress.postalCode}, {order.shippingAddress.country}
                      </div>
                    )}

                    {order.trackingNumber && (
                      <div style={{ marginTop: '15px', padding: '16px', background: '#f8fafc', borderRadius: '12px', border: '1px solid var(--border-light)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', borderBottom: '1px solid #e2e8f0', paddingBottom: '8px' }}>
                          <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                            {t('logistics')} <strong>{order.trackingNumber}</strong> ({(order.shippingCarrier || 'GHN').toUpperCase()})
                          </span>
                        ...
                        {order.shippingHistory && order.shippingHistory.length > 0 ? (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '10px', paddingLeft: '12px', borderLeft: '2px solid var(--primary-color)', position: 'relative' }}>
                            {order.shippingHistory.map((log, idx) => (
                              <div key={idx} style={{ position: 'relative', fontSize: '0.8rem', color: 'var(--text-main)' }}>
                                <div style={{
                                  position: 'absolute',
                                  left: '-17px',
                                  top: '4px',
                                  ...
                                  height: '8px',
                                  borderRadius: '50%',
                                  background: idx === order.shippingHistory.length - 1 ? 'var(--primary-color)' : '#cbd5e1',
                                  boxShadow: idx === order.shippingHistory.length - 1 ? '0 0 0 3px rgba(99, 102, 241, 0.2)' : 'none'
                                }} />
                                <div style={{ fontWeight: idx === order.shippingHistory.length - 1 ? '700' : '500' }}>
                                  [{getStatusText(log.status)}] - {log.location}
                                </div>
                          ...
                          <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{t('no_logistics_updates')}</div>
                        )}
                      </div>
                    )}

                    {/* Shopee Guarantee Escrow Actions */}
                    {order.status === 'Delivered' && order.escrowStatus === 'held' && (
                      <div style={{ display: 'flex', gap: '10px', marginTop: '15px', justifyContent: 'flex-end' }}>
                        <button
                          onClick={async () => {
                            if (window.confirm(t('order_received_confirm'))) {
                              try {
                                const res = await fetch(`${API_BASE}/api/orders/${order._id}/receive`, {
                                  method: 'PUT',
                                  headers: { "Authorization": `Bearer ${token}` }
                                });
                                if (res.ok) {
                                  alert(t('order_received_success'));
                        ...
                        <button
                          onClick={async () => {
                            ...
                            if (reason) {
                              try {
                                const res = await fetch(`${API_BASE}/api/orders/${order._id}/refund-request`, {
                                  method: 'POST',
                                  headers: {
                                ...
                                if (res.ok) {
                                  alert(t('refund_request_success'));
                                  window.location.reload();
                                } else {
                                  const d = await res.json();
                                  alert(d.error || "Action failed");
                          ...
                          style={{ padding: '8px 16px', background: '#ef4444', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.85rem' }}
                        >
                          {t('return_refund')}
                        </button>
                      </div>
                    ...
                    {order.escrowStatus && order.escrowStatus !== 'held' && (
                      <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '15px' }}>
                        <span style={{
                          fontSize: '0.85rem',
                          fontWeight: 'bold',
                          color: order.escrowStatus === 'released' ? '#10b981' : (order.escrowStatus === 'disputed' ? '#f59e0b' : '#ef4444'),
                          background: order.escrowStatus === 'released' ? '#ecfdf5' : (order.escrowStatus === 'disputed' ? '#fffbeb' : '#fef2f2'),
                          padding: '4px 10px',
                          borderRadius: '6px'
                        }}>
        ...
        {activeTab === 'addresses' && (
          <div>
            <h2 style={{ fontSize: '2rem', margin: '0 0 30px 0', fontWeight: '800', color: 'var(--text-main)' }}>{t('address_book')}</h2>
            
            {addressMsg && (
              <div style={{ padding: '10px 15px', borderRadius: '8px', marginBottom: '15px', background: addressMsg.includes('success') || addressMsg.includes('removed') || addressMsg.includes('thành công') || addressMsg.includes('xóa') ? '#f0fdf4' : '#fef2f2', color: addressMsg.includes('success') || addressMsg.includes('removed') || addressMsg.includes('thành công') || addressMsg.includes('xóa') ? '#16a34a' : '#ef4444', fontSize: '0.9rem', fontWeight: '600' }}>
                {addressMsg}
              </div>
            )}

            <div style={{ background: 'white', borderRadius: '16px', padding: '25px', border: '1px solid var(--border-light)', marginBottom: '25px' }}>
              <h4 style={{ margin: '0 0 20px 0' }}>{editingAddressId ? t('edit_address') : t('add_new_address')}</h4>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '6px', fontWeight: '600', fontSize: '0.85rem' }}>{t('address_label')}</label>
                  <select value={addressForm.label} onChange={e => setAddressForm({...addressForm, label: e.target.value})} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid var(--border-light)' }}>
                    <option value="Home">{t('home_label')}</option>
                    <option value="Work">{t('work_label')}</option>
                    <option value="Other">{t('other_label')}</option>
                  </select>
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '6px', fontWeight: '600', fontSize: '0.85rem' }}>{t('phone')}</label>
                  <input value={addressForm.phone} onChange={e => setAddressForm({...addressForm, phone: e.target.value})} style={{ width: '100%', boxSizing: 'border-box', padding: '10px', borderRadius: '8px', border: '1px solid var(--border-light)' }} placeholder={t('phone')} />
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={{ display: 'block', marginBottom: '6px', fontWeight: '600', fontSize: '0.85rem' }}>{t('street_address')}</label>
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '6px', fontWeight: '600', fontSize: '0.85rem' }}>{t('city')}</label>
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '6px', fontWeight: '600', fontSize: '0.85rem' }}>{t('postal_code')}</label>
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '6px', fontWeight: '600', fontSize: '0.85rem' }}>{t('country')}</label>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <label style={{ fontSize: '0.85rem', fontWeight: '600' }}>{t('set_as_default')}</label>
                </div>
              </div>
              <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
                <button onClick={async () => {
                  ...
                  try {
                    ...
                    if (res.ok) {
                      setAddressMsg(editingAddressId ? t('address_updated_success') : t('address_added_success'));
                  ...
                  {editingAddressId ? t('update_label') : t('add_address')}
                </button>
                {editingAddressId && (
                  <button onClick={() => { setEditingAddressId(null); setAddressForm({ label: 'Home', address: '', city: '', postalCode: '', country: '', phone: '', isDefault: false }); }} style={{ padding: '10px 24px', background: '#f1f5f9', color: 'var(--text-main)', border: 'none', borderRadius: '10px', fontWeight: '700', cursor: 'pointer' }}>{t('cancel_label')}</button>
                )}
              </div>
            </div>
            ...
            {addresses.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px', background: '#f8fafc', borderRadius: '12px', border: '2px dashed var(--border-light)' }}>
                <p style={{ color: 'var(--text-muted)' }}>{t('no_saved_addresses')}</p>
              </div>
            ) : (
              <div style={{ display: 'grid', gap: '15px' }}>
                {addresses.map(addr => (
                  <div key={addr._id} style={{ background: 'white', borderRadius: '12px', padding: '20px', border: addr.isDefault ? '2px solid var(--primary-color)' : '1px solid var(--border-light)', position: 'relative' }}>
                    {addr.isDefault && <span style={{ position: 'absolute', top: '10px', right: '10px', background: '#eef2ff', color: 'var(--primary-color)', fontSize: '0.7rem', fontWeight: '800', padding: '3px 8px', borderRadius: '99px' }}>{t('default')}</span>}
                    <div style={{ fontWeight: '700', marginBottom: '6px' }}>{addr.label === 'Home' ? t('home_label') : (addr.label === 'Work' ? t('work_label') : t('other_label'))}</div>
                    <div style={{ color: 'var(--text-muted)', fontSize: '0.9rem', lineHeight: 1.6 }}>
                      {addr.address}, {addr.city}, {addr.postalCode}, {addr.country}
                      {addr.phone && <><br/>{t('phone')}: {addr.phone}</>}
                    </div>
                    <div style={{ display: 'flex', gap: '10px', marginTop: '12px' }}>
                      <button onClick={() => { setEditingAddressId(addr._id); setAddressForm({ label: addr.label, address: addr.address, city: addr.city, postalCode: addr.postalCode, country: addr.country, phone: addr.phone || '', isDefault: addr.isDefault }); }} style={{ padding: '6px 14px', borderRadius: '6px', border: '1px solid var(--border-light)', background: 'white', fontWeight: '600', fontSize: '0.8rem', cursor: 'pointer' }}>{t('edit_label')}</button>
                      <button onClick={async () => {
                        ...
                        setAddressMsg(t('address_removed'));
                      }} style={{ padding: '6px 14px', borderRadius: '6px', border: '1px solid #fecaca', background: '#fef2f2', color: '#ef4444', fontWeight: '600', fontSize: '0.8rem', cursor: 'pointer' }}>{t('delete_label')}</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === 'settings' && (
          <div>
            <h2 style={{ fontSize: '2rem', margin: '0 0 30px 0', fontWeight: '800', color: 'var(--text-main)' }}>{t('account_settings')}</h2>
            ...
            <div style={{ background: 'white', borderRadius: '24px', padding: '30px', border: '1px solid var(--border-light)', boxShadow: '0 10px 30px rgba(0,0,0,0.02)', marginBottom: '25px' }}>
              <h4 style={{ margin: '0 0 20px 0' }}>{t('profile_details')}</h4>
              {settingsMsg && (
                <div style={{ padding: '10px 15px', borderRadius: '8px', marginBottom: '15px', background: settingsMsg.includes('success') || settingsMsg.includes('thành công') ? '#f0fdf4' : '#fef2f2', color: settingsMsg.includes('success') || settingsMsg.includes('thành công') ? '#16a34a' : '#ef4444', fontSize: '0.9rem', fontWeight: '600' }}>
                  {settingsMsg}
                </div>
              )}
              <div style={{ display: 'grid', gap: '20px' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: '600', fontSize: '0.9rem' }}>{t('full_name')}</label>
                  <input type="text" value={editName} onChange={e => setEditName(e.target.value)} style={inputStyle} />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: '600', fontSize: '0.9rem' }}>{t('email_address')}</label>
            ...
            <div style={{ background: 'white', borderRadius: '24px', padding: '30px', border: '1px solid var(--border-light)', boxShadow: '0 10px 30px rgba(0,0,0,0.02)' }}>
              <h4 style={{ margin: '0 0 20px 0' }}>{t('change_password')}</h4>
              {passwordMsg && (
                <div style={{ padding: '10px 15px', borderRadius: '8px', marginBottom: '15px', background: passwordMsg.includes('success') || passwordMsg.includes('thành công') ? '#f0fdf4' : '#fef2f2', color: passwordMsg.includes('success') || passwordMsg.includes('thành công') ? '#16a34a' : '#ef4444', fontSize: '0.9rem', fontWeight: '600' }}>
                  {passwordMsg}
                </div>
              )}
              <div style={{ display: 'grid', gap: '20px' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: '600', fontSize: '0.9rem' }}>{t('current_password')}</label>
                  <input type="password" value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} style={inputStyle} placeholder={t('current_password')} />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: '600', fontSize: '0.9rem' }}>{t('new_password')}</label>
                  <input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} style={inputStyle} placeholder={t('new_password')} />
                </div>
              </div>
