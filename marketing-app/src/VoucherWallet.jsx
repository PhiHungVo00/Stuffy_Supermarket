        ...
        {vouchers.map(v => {
          ...
          return (
            <div key={v._id} style={{ display: 'flex', border: '1px solid', borderColor: isClaimed ? '#e2e8f0' : '#c7d2fe', borderRadius: '12px', overflow: 'hidden', opacity: isClaimed ? 0.6 : 1, transition: 'all 0.3s' }}>
              ...
              <div style={{ background: isClaimed ? '#f1f5f9' : 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: isClaimed ? '#334155' : 'white', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '20px 15px', minWidth: '90px', borderRight: '2px dashed', borderRightColor: isClaimed ? '#cbd5e1' : 'rgba(255,255,255,0.4)', position: 'relative' }}>
              ...
              <div style={{ padding: '15px', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', background: isClaimed ? '#f8fafc' : 'white' }}>
                <div>
                  <div style={{ fontSize: '0.8rem', color: '#475569', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '4px' }}>{v.type}</div>
                  <div style={{ fontWeight: '700', color: 'var(--text-main)', fontSize: '1rem', lineHeight: 1.2 }}>{v.description}</div>
                  <div style={{ fontSize: '0.75rem', color: '#475569', marginTop: '4px' }}>Code: {v.code}</div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '15px' }}>
                  <div style={{ fontSize: '0.75rem', color: '#991b1b', fontWeight: '700' }}>⏰ {getTimeLeft(v.expiresAt)}</div>
                  <button 
                    onClick={() => handleClaim(v)}
                    disabled={isClaimed}
                    style={{ background: isClaimed ? 'transparent' : '#fef2f2', color: isClaimed ? '#475569' : '#991b1b', border: isClaimed ? 'none' : '1px solid #fca5a5', padding: '5px 12px', borderRadius: '6px', fontWeight: 'bold', cursor: isClaimed ? 'not-allowed' : 'pointer', fontSize: '0.8rem' }}
                  >
